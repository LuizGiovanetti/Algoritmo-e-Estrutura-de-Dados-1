...
Em seguida, uma matriz deve ser lida. Por último, a matriz bidimensional deve ser convertida para uma matriz esparsa.
...
int matriz_triangular_sup(Spa_Mat *m)
{
    if(m != NULL)
    {
...
int matriz_triangular_inf(Spa_Mat *m)
{
    if(m != NULL)
    {
        for(int i = 0; i < m->n_lin; i++)
        {
            ...
            if((aux != NULL) && ((aux->col > i) || (aux->next != NULL)))
                return 0;
        }
        return 1;
    }
...
int matriz_triangular(Spa_Mat *m)
{
    return matriz_triangular_sup(m) || matriz_triangular_inf(m);
}
