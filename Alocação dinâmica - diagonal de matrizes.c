...
A função deverá retornar um vetor (ponteiro) alocado dinamicamente com os elementos da diagonal principal da matriz.
...
int main() {
    ...
    printf("\n");
    free(vetor); // liberando a memória alocada dinamicamente para o vetor
    for (i = 0; i < n; i++) {
        free(matriz[i]); // liberando a memória alocada dinamicamente para cada linha da matriz
    }
    free(matriz); // liberando a memória alocada dinamicamente para a matriz
    return 0;
}
