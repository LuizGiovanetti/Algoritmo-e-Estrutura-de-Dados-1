/*
Implemente uma função que receba uma matriz esparsa como parâmetro.
A função deve retornar o resultado da soma entre os elementos da diagonal principal da matriz esparsa.

Input Format
Na primeira linha deve ser lido um número inteiro, que representará os números de linhas e de colunas (matriz quadrada).
Em seguida, uma matriz deve ser lida. A matriz bidimensional deve ser convertida para uma matriz esparsa.
...
int somar_diagonal(Spa_Mat *m)
{
    ...
    if(m != NULL)
    {
        for(int i = 0; i < m->n_lin; i++)
        {
            ...
            while((aux != NULL) && (aux->col < i))
                aux = aux->next;

            if((aux != NULL) && (aux->col == i))
               soma += aux->item;
        }
    }
